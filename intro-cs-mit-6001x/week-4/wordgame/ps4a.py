...
def isValidWord(word, hand, wordList):
    ...
    """
    # Check if a word is not in wordList
    if word not in wordList:
        return False
    
    # Check to make sure the word is entirely composed of letters in the hand
    new_hand = hand.copy()
    for char in word:

        # If there is a letter that is not in hand then return false
        if char not in hand:
            return False
        
        # Update the count and check to see if each letter is used only once
        new_hand[char] -= 1
        if new_hand[char] < 0:
            return False
        
    # If pass all the tests above then return true
    return True
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(f'"{user_word}" earned {score} points. Total: {total_score} points.')
                print()
