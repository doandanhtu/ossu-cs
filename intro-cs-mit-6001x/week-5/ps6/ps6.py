...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        target_lowercase = lowercase_char * 2
        for i, char in enumerate(lowercase_char):
            shift_dict[char] = target_lowercase[i + shift]
        
        uppercase_char = string.ascii_uppercase
        target_uppercase = uppercase_char * 2
        for i, char in enumerate(uppercase_char):
            shift_dict[char] = target_uppercase[i + shift]
        return shift_dict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(25):
            ...
            check_message = decrypted_message.split()
            for word in check_message:
                if is_word(self.valid_words, word):
                    real_words_count += 1
            if real_words_count > best_words_count:
                best_words_count = real_words_count
                best_shift = s
